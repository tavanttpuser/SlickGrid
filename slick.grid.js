    ...
    throw "slick.core.js not loaded";
        ...
        function SlickGrid(container, data, columns, options) {
            ...
            function init() {
                ...
                $headerRowSpacerL = $("<div style='display:block;height:1px;position:absolute;top:0;left:0;'></div>").css("width", getCanvasWidth() + scrollbarDimensions.width + "px").appendTo($headerRowScrollerL);
                $headerRowSpacerR = $("<div style='display:block;height:1px;position:absolute;top:0;left:0;'></div>").css("width", getCanvasWidth() + scrollbarDimensions.width + "px").appendTo($headerRowScrollerR);
            ...
            function finishInitialization() {
                if (!initialized) {
                    ...
                    /*      .bind("draginit", handleDragInit)
                     ...
                     .bind("dragend", handleDragEnd)*/.delegate(".slick-cell", "mouseenter", handleMouseEnter).delegate(".slick-cell", "mouseleave", handleMouseLeave);
                }
            }
            ...
            function updateCanvasWidth(forceColumnWidthsUpdate) {
                ...
                if (widthChanged || options.frozenColumn > -1 || hasFrozenRows) {
                    ...
                    if (options.frozenColumn > -1) {
                        ...
                        var parentContainerWidth = parseFloat($.css($container[0], "width", true));

                        $viewportTopR.width(parentContainerWidth - canvasWidthL);
                        if (parentContainerWidth < canvasWidthL) {
                            trigger(self.onUnfitFrozenColumn, {});
                        }
                        if (hasFrozenRows) {
                ...
                var scrollWidth = 0;
                if (options.enableVScroll && viewportHasVScroll) {
                    scrollWidth = scrollbarDimensions.width;
                }
                viewportHasHScroll = (canvasWidth > viewportW - scrollWidth) && options.enableHScroll;
            ...
            function setupColumnSort() {
                $headers.click(function(e) {
                    ...
                    if ($(e.target).hasClass("datafilter")) {
                        return;
                    }
            ...
            function setFrozenOptions() {
                ...
                ) ? parseInt(options.frozenColumn) : -1;

                options.frozenRow = (options.frozenRow >= 0 && options.frozenRow < numVisibleRows
                ) ? parseInt(options.frozenRow) : -1;
            ...
            function getCacheRowPositions() {
                return rowPositionCache;
            }
            ...
            function cacheRowPositions() {
                ...
                for (var i = 0; i <= getDataLength(); i++) {
                    ...
                    rowPositionCache[i] = {
                        ...
                        height : (metadata && metadata.height != undefined) ? metadata.height : options.rowHeight
                    }
            ...
            function appendRowHtml(stringArrayL, stringArrayR, row, range) {
                ...
                var frozenRowOffset = (hasFrozenRows ) ? (options.frozenBottom ) ? (row >= actualFrozenRow ) ? (h < viewportTopH ) ? (actualFrozenRow * rowPositionCache[row].height ) : h : 0 : (row >= actualFrozenRow ) ? frozenRowsHeight : 0 : 0;

                var tempTotalHeight = 0;
                for (var rowKey in rowPositionCache) {
                    if (Number(rowKey) === row) {
                        break;
                    }
                    tempTotalHeight += rowPositionCache[rowKey].height;
                }
                var rowHtml = "<div class='ui-widget-content " + rowCss + "' style='top:" + (tempTotalHeight - offset - frozenRowOffset ) + "px;";

                stringArrayL.push(rowHtml);
                stringArrayL.push((rowPositionCache[row].height != options.rowHeight ) ? "height:" + rowPositionCache[row].height + "px;'>" : "'>");

                if (options.frozenColumn > -1) {
                    stringArrayR.push(rowHtml);
                    stringArrayR.push((rowPositionCache[row].height != options.rowHeight ) ? "height:" + rowPositionCache[row].height + "px;'>" : "'>");
                }
            ...
            function appendCellHtml(stringArray, row, cell, colspan) {
                ...
                stringArray.push("'");
                stringArray.push((rowPositionCache[row].height != options.rowHeight ) ? "style='height:" + (rowPositionCache[row].height - cellHeightDiff ) + "px;'" : "");
                stringArray.push(">");
            ...
            function updateRow(row) {
                ...
                for (var columnIdx in cacheEntry.cellNodesByColumnIdx) {
                    ...
                    var m = columns[columnIdx], d = getDataItem(row), node = cacheEntry.cellNodesByColumnIdx[columnIdx][0];
            ...
            function getViewportHeight() {
                if (options.autoHeight) {
                    ...
                    viewportH = parseFloat($.css($container[0], "height", true)) - parseFloat($.css($container[0], "paddingTop", true)) - parseFloat($.css($container[0], "paddingBottom", true)) - parseFloat($.css($headerScroller[0], "height")) - getVBoxDelta($headerScroller) - topPanelH - headerRowH;
                }
            ...
            function updateRowCount() {
                ...
                var tempTotalHeight = 0;
                for (var rowKey in rowPositionCache) {
                    if (Number(rowKey) === numberOfRows) {
                        break;
                    }
                    tempTotalHeight += rowPositionCache[rowKey].height;
                }
                // with autoHeight, we do not need to accommodate the vertical scroll bar
                viewportHasVScroll = !options.autoHeight && (tempTotalHeight > viewportH) && options.enableVScroll;
                ...
                th = Math.max(tempTotalHeight, tempViewportH - scrollbarDimensions.height);
            ...
            function renderRows(range) {
                var stringArrayL = [], stringArrayR = [], rows = [], needToReselectCell = false;
                ...
                for (var i = 0, ii = rows.length; i < ii; i++) {
                    if ((hasFrozenRows ) && (rows[i] >= actualFrozenRow )) {
                        if (options.frozenColumn > -1) {
                            rowsCache[rows[i]].rowNode = $().add($(x.firstChild).appendTo($canvasBottomL)).add($(xRight.firstChild).appendTo($canvasBottomR));
                        } else {
                            rowsCache[rows[i]].rowNode = $().add($(x.firstChild).appendTo($canvasBottomL));
                        }
                    } else if (options.frozenColumn > -1) {
                        rowsCache[rows[i]].rowNode = $().add($(x.firstChild).appendTo($canvasTopL)).add($(xRight.firstChild).appendTo($canvasTopR));
                    } else {
            ...
            function updateRowPositions() {
                for (var row in rowsCache) {
                    rowsCache[row].rowNode.css('top', (row * rowPositionCache[row].height - offset) + "px");
                }
            }
                    ...
                    if (!e.shiftKey && !e.altKey && !e.ctrlKey) {
                        if (e.which == 27) {
                            if (!getEditorLock().isActive()) {
                                return;
                                // no editing mode to cancel, allow bubbling and default processing (exit without cancelling the event)
                            }
            ...
            function handleClick(e) {
                ...
                if ((activeCell != cell.cell || activeRow != cell.row) && canCellBeActive(cell.row, cell.cell)) {
                    if (!getEditorLock().isActive() || getEditorLock().commitCurrentEdit()) {
                        if (hasFrozenRows) {
                            if ((!(options.frozenBottom ) && (cell.row >= actualFrozenRow ) ) || (options.frozenBottom && (cell.row < actualFrozenRow ) )) {
            ...
            function scrollCellIntoView(row, cell) {
                ...
                var left = columnPosLeft[cell], right = columnPosRight[cell + (colspan > 1 ? colspan - 1 : 0)], scrollRight = scrollLeft + $viewportScrollContainerX.width();
            ...
            function getViewPortScrollInfo() {
                var vp = {};
                vp.HScroll = viewportHasHScroll;
            ...
            $.extend(this, {
                ...
                "onUnfitFrozenColumn" : new Slick.Event(),
                ...
                "setFrozenOptions" : setFrozenOptions,
                "getData" : getData,
                ...
                "cacheRowPositions" : cacheRowPositions,
                "getCacheRowPositions" : getCacheRowPositions,
                "updateRow" : updateRow,
